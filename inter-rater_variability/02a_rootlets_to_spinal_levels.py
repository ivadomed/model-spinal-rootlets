...
The script does the following:
    - project the nerve rootlets on the spinal cord segmentation to obtain spinal levels. This is done by dilating the spinal cord segmentation by 1, 2 or 3 voxels ("-dilate" input argument) and then finding 
    the intersection between the dilated spinal cord segmentation and the rootlets segmentation.
...
def project_rootlets_to_segmentation(im_rootlets, im_seg, im_intersect, rootlets_levels, fname_rootlets, level_type):
    ...
    for level in rootlets_levels:
        ...
        if level_type == 'rootlets':
            ...
            # Keep only slices with a single value (e.g., `2`). This is done because slices between the adjacent
            # vertebral levels might have two values; for example, a slice between C2-C3 vertebral levels might have
            # values `2` and `3`.
