...
def generate_figure(df, dir_path):
    """
    Generate a figure showing the inter-rater variability for individual sessions and spinal levels.
    ...
    for x, session in enumerate(df['session'].unique(), 1):
        ...
        for level in df['spinal_level'].unique():
            ...
            # Plot rectangle for the session, rater and spinal level
            ax.add_patch(
                patches.Rectangle(
                    (x, start),      # (x,y)
                    0.1,            # width
...
def compute_mean_and_COV(df, dir_path):
    ...
    # Reformat the DataFrame to have spinal_levels as rows and sessions and raters as columns
    ...
    fname_csv = 'table_inter_rater_variability-courtois-neuromod.csv'
