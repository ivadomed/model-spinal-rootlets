...
def calculate_overlap(df, output_path):
    # get unique subjects:
...
def create_kdeplot(df_overlap, output_path):
    sns.set_palette('muted')
    g = sns.FacetGrid(df_overlap, col='spinal_level_name', hue='sex', col_wrap=3, height=4, ylim=(0,100))
    g.map(sns.kdeplot, 'age', 'overlap', fill=True)
    plt.subplots_adjust(top=0.9)
    g.fig.suptitle('KDE plots of Overlap by Age, Sex, and Spinal Level')
    plt.savefig(f'{output_path}/kdeplot_overlap_rootlets_vertebrae.svg', dpi=300)
    plt.show()
...
def main():
    ...
    overlap_table = calculate_overlap(df, output_path)
    create_boxplot(overlap_table, output_path)
    create_scatterplot(overlap_table, output_path)
    create_kdeplot(overlap_table, output_path)
