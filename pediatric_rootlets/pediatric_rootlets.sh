...
if [ -f ${file_t2_composed}.nii.gz ]; then
  $SCT_DIR/python/envs/venv_sct/bin/python ~/code/model-spinal-rootlets/pediatric_rootlets/crop_composed_images.py -i_composed ${file_t2_composed}.nii.gz -i_top ${file_t2_top}.nii.gz -i_composed_label ${file_t2_composed}.nii.gz -s ${FILESEG}.nii.gz -d ${FILELABEL}.nii.gz -pmj ${FILEPMJ}.nii.gz -rootlets_seg ${file_t2}_label-rootlets_dseg.nii.gz
  ...
  $SCT_DIR/python/envs/venv_sct/bin/python ~/code/model-spinal-rootlets/inter-rater_variability/02a_rootlets_to_spinal_levels.py -i ${file_t2}_label-rootlets_dseg_crop.nii.gz -s ${file_t2}_label-SC_mask_crop.nii.gz -pmj ${file_t2}_label-PMJ_dlabel_crop.nii.gz -type rootlets
  ...
  $SCT_DIR/python/envs/venv_sct/bin/python ~/code/model-spinal-rootlets/pediatric_rootlets/vertebrae_to_spinal_levels.py -centerline ${file_t2}_label-SC_mask_centerline_extrapolated.csv -disclabel ${file_t2}_labels-disc_crop.nii.gz
