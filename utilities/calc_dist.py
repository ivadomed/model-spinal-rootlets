...
def main():
    ...
    for j in range(len(nerves)):
        ...
        if nerve in discs:
            ...
            disc_index_corr = np.abs(centerline[2] - discs_index[discs.index(
                nerve)]).argmin()  # centerline doesn't necessarly start at the index 0 if the segmentation is incomplete
            nerve2_index_corr = np.abs(centerline[2] - nerve_index[j]).argmin()
            distance_disc_nerve = arr_distance[:, disc_index_corr][0] - arr_distance[:, nerve2_index_corr][
                0]  # Disc to nerve
        else:
